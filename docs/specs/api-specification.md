...
| `GET` | `/api/documents/{id}/export` | editor | 単一文書を JSON エンベロープにエクスポート |
| `POST` | `/api/documents/import` | editor | エンベロープから新規文書を作成 (インポート) |
...
### 4-bis. 単一文書のエクスポート / インポート

管理者向けの DB 全体バックアップ (`/api/backups`) とは別に、editor が 1 文書単位で持ち出し・取り込みできる。

- `GET /api/documents/{id}/export` はポータブルな JSON エンベロープを返す。形式:

```json
{
  "doc_platform_export": 1,
  "exported_at": "...",
  "document": {
    "title": "...", "slug": "...", "summary": "...", "content_markdown": "...",
    "category_name": "...", "lesson_name": "...", "tag_names": ["..."], "plugin_data": {}
  },
  "revisions": [{"version_number": 1, "summary": "...", "content_markdown": "...", "created_at": "..."}],
  "attachments": [{"id": 1, "file_name": "...", "mime_type": "...", "size_bytes": 0, "data_base64": "..."}]
}
```

  - 分類 (カテゴリ/レッスン/タグ) は ID ではなく**名前**で持ち出すため、別インスタンスへ移しても再解決できる
  - 添付画像はディスクから読み込み base64 で同梱する。各添付は**元の id** を保持し、インポート側で URL を書き換える
- `POST /api/documents/import` はエンベロープを受け取り、**常に新規文書を作成**する
  - slug が衝突した場合は `-copy` / `-copy-2` … と一意化する
  - `category_name` / `lesson_name` / `tag_names` は名前 (大文字小文字無視) で既存を検索し、なければ作成する
  - 画像添付を再作成し、本文と各リビジョンの `/api/attachments/{旧id}` を `/api/attachments/{新id}` に書き換える
  - 応答に作成された文書を含む。ディスクへ復元できなかった添付は `skipped_attachments` に列挙する
  - エンベロープ不正時は `validation_error`
...
| Method | Path | 権限 | 用途 |
| --- | --- | --- | --- |
| `GET` | `/api/documents/{id}/attachments` | editor | 文書の添付一覧 (未削除のみ) |
| `POST` | `/api/documents/{id}/attachments` | editor | 添付追加 |
| `GET` | `/api/attachments/{id}` | viewer | 添付参照 |
| `DELETE` | `/api/attachments/{id}` | editor | 添付削除 |

- 一覧 (`GET /api/documents/{id}/attachments`) は未削除の添付を新しい順に返す。各項目は `id`、`file_name`、`mime_type`、`size_bytes`、`created_at`、`url` (`/api/attachments/{id}`)
- 削除 (`DELETE /api/attachments/{id}`) は DB レコードを論理削除すると同時に、ディスク上の実ファイルも best-effort で削除する (失敗しても 204 を返す)。依存コメントを残すため DB 行自体は保持する
...
差分応答 (`/api/revisions/{id}/diff`):

- `target` (新しい側) / `against` (古い側) / 従来の unified `diff` 文字列に加え、左右並列描画用の `rows` 配列を返す
- `rows` は difflib の opcode を整列した行配列で、各行は `{type, left_no, left, right_no, right}` (`type` は `equal` / `insert` / `delete` / `replace`、行番号は 1 始まり、片側のみの行は `null`)
...
| `GET` | `/api/glossary/revisions/diff?a=&b=` | viewer | 用語の説明 diff (`a`=新, `b`=旧。`diff`/`rows`/`left_version`/`right_version` を返す) |
